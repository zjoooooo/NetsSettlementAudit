...
{
    public class ShareFolderConnect
    {
        public ShareFolderConnect() { }
   ...
   /**
    public partial class _Default : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //连接共享文件夹
            status = ShareFolderConnect.connectState(@"\\10.80.88.180\test", "admin", "admin");
            if (status)
            {
                ...
                ShareFolderConnect.ReadFiles(filename);

                //测试写文件，拼出完整的路径
                filename = theFolder.ToString() + "\\bad.txt";
                ShareFolderConnect.WriteFiles(filename);
